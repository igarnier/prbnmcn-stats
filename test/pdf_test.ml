  ...
  @@ QCheck.Test.make
       ...
       QCheck.(
         triple
           ...
           (float_range 0.1 10.))
  @@ fun (mean, std, x) ->
  let ln_version = Log_space.to_float (Pdfs.gaussian_ln ~mean ~std x) in
...
let () =
  ...
  @@ fun (rate, x) ->
  let ln_version = Log_space.to_float (Pdfs.exponential_ln ~rate x) in
...
let () =
  ...
  @@ fun (k, p) ->
  let ln_version = Log_space.to_float (Pdfs.geometric_ln ~p k) in
...
let () =
  ...
  @@ fun (k, lambda) ->
  let ln_version = Log_space.to_float (Pdfs.poisson_ln ~lambda k) in
...
let () =
  ...
  let ln_version = Log_space.to_float (Pdfs.uniform_ln { min; max } x) in
