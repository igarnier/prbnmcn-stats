...
let () =
  ...
  let approx =
    ...
    |> Fin.Float.normalize
  in
  let (`Probability empirical) = Fin.Float.raw_data_probability approx in
  let theoretical =
    Fin.Float.measure (module Basic_impl.Free_module.Float_valued.Int)
    @@ List.map (fun (x, _) -> (x, Pdfs.poisson ~lambda ~k:x)) empirical
    |> Fin.Float.normalize
  in
  let kl_div =
    Fin.Float.Dist.kl (Fin.as_measure approx) (Fin.as_measure theoretical)
  in
  kl_div <. 0.01
