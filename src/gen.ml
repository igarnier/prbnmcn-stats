...
struct
  ...
  let rectangle ~min ~max state =
    if Array.length min <> Array.length max then
      invalid_arg "rectangle: Array.length min <> Array.length max" ;
    if Array.length min = 0 then invalid_arg "rectangle: min has zero length" ;
    let deltas = Array.map2 (fun x y -> y -. x) min max in
    if Array.exists (fun delta -> delta <. 0.0) deltas then
      invalid_arg "rectangle: invalid box, some dimension is empty" ;
    Array.mapi (fun i d -> min.(i) +. RNG.float state d) deltas
  ...
  module Float = struct
    ...
    let tup2 g1 g2 state = (g1 state, g2 state)

    let tup3 g1 g2 g3 state = (g1 state, g2 state, g3 state)

    let tup4 g1 g2 g3 g4 state = (g1 state, g2 state, g3 state, g4 state)

    let tup5 g1 g2 g3 g4 g5 state =
      (g1 state, g2 state, g3 state, g4 state, g5 state)

    let tup6 g1 g2 g3 g4 g5 g6 state =
      (g1 state, g2 state, g3 state, g4 state, g5 state, g6 state)

    let mixture coeffs gens =
      if Array.length coeffs = 0 then invalid_arg "mixture: empty coeffs" ;
      let sum = ref 0.0 in
      for i = 0 to Array.length coeffs - 1 do
        let c = coeffs.(i) in
        if c <. 0.0 then invalid_arg "mixture: negative weight" ;
        sum := !sum +. coeffs.(i)
      done ;
      if abs_float (1. -. !sum) >. 0.001 then
        invalid_arg "mixture: unnormalized weights" ;
      let sampler = categorical (Array.mapi (fun i c -> (i, c)) coeffs) in
      fun rng_state ->
